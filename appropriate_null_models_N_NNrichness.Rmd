...
The slope parameter for the effect of native richness on psi is b1; this indicates there is very strong evidence that native and nonnative richness are positively related, beyond what would be expected by random sampling. Using this framework one could easily add in additional predictors of psi, as well as random effects. It would be interesting to see how this changes, for example, with plot size, ecoregion, etc.
...
This shows that the hypergeometric model is indeed an appropriate null model. The last step is to confirm no bias in values of psi using this randomized dataset:
...
Although it varies based on the random seed of the random shuffling, generally b1 overlaps zero, suggesting the hypergeometric approach is a sound null model for native-nonnative richness data. We should probably confirm using many more shuffling permutations.
...
Note here is Beaury's mixed model version:
